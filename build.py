...
portrait = f'<div class="portrait"><img src="{e(image)}" alt="銀青のボブ、白いねこ耳、ピンクのワンピース姿のちび猫ちゃん" width="1122" height="1402" fetchpriority="high"></div>' if image else ''
social_icons = {
    'X':'<span class="x-mark" aria-hidden="true">𝕏</span>',
    'YouTube':'<svg class="youtube-mark" viewBox="0 0 48 34" aria-hidden="true"><rect x="2" y="2" width="44" height="30" rx="9" fill="currentColor"/><path d="M20 10l11 7-11 7z" fill="#fff"/></svg>',
    'TikTok':'<svg class="tiktok-mark" viewBox="0 0 44 44" aria-hidden="true"><path d="M27 6v17.5a8.5 8.5 0 1 1-5.2-7.8" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round"/><path d="M27 6c1.5 5 4.4 7.5 9 8" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round"/></svg>'
}
links = ''.join(f'<a class="button social-icon social-{s["label"].lower()}" href="{e(s["url"])}" aria-label="{e(s["label"])}を開く">{social_icons.get(s["label"], e(s["label"]))}</a>' for s in socials)
social_section = f'<section class="social wrap" id="social"><p class="eyebrow">FOLLOW</p><p class="social-main">SNSで待ってるにゃ🐾</p><div class="social-links">{links}</div><p class="social-note">ちび猫ちゃんは、たまごぼーろのSNSで活躍中🐾</p></section>' if socials else ''
